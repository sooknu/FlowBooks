import { db } from '../db';
import { projects, projectNotes, projectAssignments, teamMembers, user, notifications } from '../db/schema';
import { eq, ilike, or, and, asc as ascFn, desc as descFn, count, ne, exists, sql } from 'drizzle-orm';
...
export default async function projectRoutes(fastify: any) {
  ...
  fastify.get('/', async (request: any) => {
    ...
    const [data, [{ total }]] = await Promise.all([
      db.query.projects.findMany({
        ...
        with: { client: true, projectTypeRel: true },
        orderBy: orderFn,
        limit: take,
        offset: skip,
      }),
  ...
  fastify.put('/:id', async (request: any, reply: any) => {
    ...
    // Check ownership + lock — privileged can always edit, others need to be creator AND not locked
    if (!isPrivileged) {
      const [existing] = await db.select({ userId: projects.userId, lockedBy: projects.lockedBy }).from(projects).where(eq(projects.id, request.params.id));
      ...
      if (existing.lockedBy) {
        return reply.code(403).send({ error: 'This project has been locked by a manager' });
      }
    }
  ...
  // PUT /api/projects/:id/lock — locks a project
  fastify.put('/:id/lock', { preHandler: [requirePermission('lock_projects')] }, async (request: any, reply: any) => {
    const [existing] = await db.select({ title: projects.title }).from(projects).where(eq(projects.id, request.params.id));
    if (!existing) return reply.code(404).send({ error: 'Not found' });

    const [data] = await db
      .update(projects)
      .set({ lockedBy: request.user.id, updatedAt: new Date() })
      .where(eq(projects.id, request.params.id))
      .returning();

    logActivity({ ...actorFromRequest(request), action: 'locked', entityType: 'project', entityId: data.id, entityLabel: existing.title });
    return { data };
  });

  // PUT /api/projects/:id/unlock — unlocks a project
  fastify.put('/:id/unlock', { preHandler: [requirePermission('lock_projects')] }, async (request: any, reply: any) => {
    const [existing] = await db.select({ title: projects.title }).from(projects).where(eq(projects.id, request.params.id));
    if (!existing) return reply.code(404).send({ error: 'Not found' });
    ...
    logActivity({ ...actorFromRequest(request), action: 'unlocked', entityType: 'project', entityId: data.id, entityLabel: existing.title });
    return { data };
  });

  // POST /api/projects/:id/request-unlock — non-privileged user requests unlock
  fastify.post('/:id/request-unlock', async (request: any, reply: any) => {
    const [existing] = await db
      .select({ title: projects.title, lockedBy: projects.lockedBy })
      .from(projects)
      .where(eq(projects.id, request.params.id));

    if (!existing) return reply.code(404).send({ error: 'Not found' });
    if (!existing.lockedBy) return reply.code(400).send({ error: 'Project is not locked' });

    // Skip if there's already an unread unlock request for this project
    const [pendingRequest] = await db
      .select({ id: notifications.id })
      .from(notifications)
      .where(and(
        eq(notifications.type, 'unlock_request'),
        eq(notifications.entityId, request.params.id),
    ...
    // Notify the user who locked it + all privileged users
    const privilegedIds = await getPrivilegedUserIds();
    const recipientIds = new Set(privilegedIds);
    recipientIds.add(existing.lockedBy);
    // Don't notify the requester themselves
    recipientIds.delete(request.user.id);

    const requesterName = request.userDisplayName || request.user.email;

    notifyUsers({
      userIds: Array.from(recipientIds),
      type: 'unlock_request',
      title: 'Unlock request',
      message: `${requesterName} wants to edit "${existing.title}"`,
      entityType: 'project',
      entityId: request.params.id,
    });

    logActivity({ ...actorFromRequest(request), action: 'requested unlock', entityType: 'project', entityId: request.params.id, entityLabel: existing.title });
    return { success: true };
  });
